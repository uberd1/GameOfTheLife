...
from PyQt6.QtWidgets import QListWidget, QInputDialog, QTabWidget, QFileDialog, QMessageBox, QStyle, QLabel, \
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QDialog,QDialogButtonBox, QComboBox
...
TRANSLATIONS = {
    'ru': {
        ...
        'menu_file': "&Файл",
        'menu_help': "&Помощь",
        'act_save': "Сохранить паттерн...",
        'act_load': "Загрузить паттерн...",
        'act_lib': "Библиотека паттернов...",
        'act_help': "Справка",
        'btn_start': "Старт",
        'btn_stop': "Стоп",
        'btn_reset': "Сброс (Глайдер)",
        'btn_clear': "Очистить",
        'msg_clear_title': "Подтверждение очистки",
        'msg_clear_text': "Вы уверены, что хотите очистить все поле?",
        'lib_title': "Библиотека паттернов",
        'lib_avail': "Доступные паттерны:",
        'btn_load_sel': "Загрузить выбранный",
        'btn_save_cur': "Сохранить текущий паттерн",
        'btn_del_sel': "Удалить выбранный",
        'input_save_title': "Сохранить паттерн",
        'input_save_label': "Введите имя паттерна:",
        'msg_del_confirm': "Вы уверены, что хотите удалить паттерн '{}'?",
        ...
        'html_controls': """
            <h3>Управление</h3>
            <ul>
                <li><b>Левая кнопка мыши:</b> Переместить курсор.</li>
                <li><b>Правая кнопка мыши:</b> Панорамирование камеры.</li>
                <li><b>Колесо мыши:</b> Зум.</li>
                <li><b>Enter:</b> Поставить/Убрать клетку.</li>
            </ul>
        """,
        'html_rules': "<h3>Правила</h3><ol><li>Выживание: 2-3 соседа.</li><li>Смерть: <2 или >3.</li><li>Рождение: 3 соседа.</li></ol>",
        'html_about': "<h3>Игра «Жизнь»</h3><p>Версия 1.0<br>Автор: uberd1</p>"
    },
    'en': {
        'window_title': "Game of Life",
        'menu_file': "&File",
        'menu_help': "&Help",
        'act_save': "Save Pattern...",
        'act_load': "Load Pattern...",
        'act_lib': "Pattern Library...",
        'act_help': "Help",
        'btn_start': "Start",
        'btn_stop': "Stop",
        'btn_reset': "Reset (Glider)",
        'btn_clear': "Clear",
        'msg_clear_title': "Clear Confirmation",
        'msg_clear_text': "Are you sure you want to clear the whole grid?",
        'lib_title': "Pattern Library",
        'lib_avail': "Available patterns:",
        'btn_load_sel': "Load Selected",
        'btn_save_cur': "Save Current Pattern",
        'btn_del_sel': "Delete Selected",
        'input_save_title': "Save Pattern",
        'input_save_label': "Enter pattern name:",
        'msg_del_confirm': "Are you sure you want to delete pattern '{}'?",
        ...
        'html_controls': """
            <h3>Controls</h3>
            <ul>
                <li><b>LMB:</b> Move cursor.</li>
                <li><b>RMB (Drag):</b> Pan view.</li>
                <li><b>Mouse Wheel:</b> Zoom.</li>
                <li><b>Enter:</b> Toggle cell.</li>
            </ul>
        """,
        'html_rules': "<h3>Rules</h3><ol><li>Survival: 2-3 neighbors.</li><li>Death: <2 or >3.</li><li>Birth: 3 neighbors.</li></ol>",
        'html_about': "<h3>Game of Life</h3><p>Version 1.0<br>Author: uberd1</p>"
    }
}
###Конец файла переводов

## - - - Класс выбора языка
# Отвечает за перевод текста в правильный язык.
class LanguageSelectDialog(QDialog):
    def __init__(self):
        ...
        label = QLabel("Choose language / Выберите язык:")
        layout.addWidget(label)

        # Выпадающий список
        self.combo = QComboBox()
        self.combo.addItem("Русский", "ru")  # "ru" - это userData
        self.combo.addItem("English", "en")  # "en" - это userData
        layout.addWidget(self.combo)

        # Кнопка ОК
        btn_box = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok)
        btn_box.accepted.connect(self.accept)  # Закрывает окно с кодом Accepted
        layout.addWidget(btn_box)

        self.selected_lang = 'ru'  # По умолчанию

    def accept(self):
        # Сохраняем выбор перед закрытием
...
class PatternLibraryWindow(QWidget):
    ...
    def __init__(self, current_cells, lang='ru'):
        super().__init__()
        self.lang = lang
        self.setWindowTitle(TRANSLATIONS[self.lang]['lib_title'])
        ...
        # Кнопки управления
        load_button = QPushButton(TRANSLATIONS[self.lang]['btn_load_sel'])
    ...
    def save_current_pattern(self):
        ...
        name, ok = QInputDialog.getText(self, tr['input_save_title'], tr['input_save_label'])
        if ok and name:
    ...
    def delete_selected_pattern(self):
        ...
        reply = QMessageBox.question(self, "Confirm", t['msg_del_confirm'].format(selected_item.text()))
...
class HelpWindow(QWidget):
    def __init__(self, lang='ru'):
        ...
        self.lang = lang
        t = TRANSLATIONS[self.lang]
        self.setWindowTitle(t['act_help'])
    ...
    def _create_controls_tab(self):
        ...
        text = TRANSLATIONS[self.lang]['html_controls']
        label.setText(text)

        controls_tab = self._create_tab(label)
        self.tabs.addTab(controls_tab, TRANSLATIONS[self.lang]['tab_controls'])
    ...
    def _create_rules_tab(self):
        ...
        label.setTextFormat(Qt.TextFormat.RichText)
        label.setWordWrap(True)
        text = TRANSLATIONS[self.lang]['html_rules']
        label.setText(text)
    ...
    def _create_about_tab(self):
        ...
        text = TRANSLATIONS[self.lang]['html_about']
...
class GameOfLifeWindow(QMainWindow):
    def __init__(self, lang = 'ru'):
        ...
        start_button = QPushButton(self.t['btn_start'])
        start_button.clicked.connect(self.start_game)
        stop_button = QPushButton(self.t['btn_stop'])
        stop_button.clicked.connect(self.stop_game)
        reset_glider_button = QPushButton(self.t['btn_reset'])
        reset_glider_button.clicked.connect(self.reset_glider)
        clear_button = QPushButton(self.t['btn_clear'])
    ...
    def _create_menu_bar(self):
        ...
        # МЕНЮ "ФАЙЛ"
        file_menu = menu_bar.addMenu(self.t['menu_file'])

        # Действие "Сохранить"
        save_action = QAction(self.t['act_save'], self)
        save_action.triggered.connect(self.save_pattern)
        file_menu.addAction(save_action)

        # Действие "Загрузить"
        load_action = QAction(self.t['act_load'], self)
        ...
        help_action = QAction(help_icon, self.t['act_help'], self)
        help_action.triggered.connect(self.show_help_window)
        help_menu = menu_bar.addMenu(self.t['menu_help'])
        help_menu.addAction(help_action)

        # Библиотекарь sqlite
        library_action = QAction(self.t['lib_title'], self)
    ...
    def show_help_window(self):
        ...
        if self.help_win is None:
            self.help_win = HelpWindow(lang=self.lang)
    ...
    def show_pattern_library(self):
        ...
        self.library_win = PatternLibraryWindow(current_cells, lang=self.lang)
    ...
    def clear(self):
        ...
        reply = QMessageBox.question(
            self,
            self.t['msg_clear_title'],
            self.t['msg_clear_text'],
...
if __name__ == "__main__":
    ...
    # exec() запускает окно в модальном режиме. Возвращает True (Accepted), если нажали ОК.
    if lang_dialog.exec():
        selected_lang = lang_dialog.get_lang()

        # 2. Передаем выбранный язык в главное окно
        window = GameOfLifeWindow(lang=selected_lang)
        window.show()

        sys.exit(app.exec())
    else:
        # Если закрыли окно выбора языка, просто выходим
        sys.exit(0)
